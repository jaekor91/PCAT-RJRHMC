...
class sampler(object):
	def __init__(self, Nobjs_max = 1000, Nsteps=10, Niter=100,\
		...
		self.moves = np.ones(Niter+1, dtype=int) * -1 # Record what sort of proposals were made.
	...
	def do_inference(self, delta=1e-6, counter_max = 1000, verbose=None):
		"""
	...
	def print_accept_rate(self):
		...
		for i in range(5):
			...
			Ntot += tally[0]
		print "Total: %d/%d (%.2f)" % (self.A.sum(), Ntot, self.A.sum()/float(Ntot))
		print "\n"
